...
for cluster_idx=1:length(pial_clusters)
    ...
    cluster.bc_f_diff=cluster.f_diff-repmat(base_fdiff,1,size(cluster.f_diff,2));
...
save(fullfile(data_dir, 'invert_burst_tc_results.mat'), 'invert_burst_tc_results');

fig=figure();
subplot(2,1,1);
hold all
for cluster_idx=1:length(new_pial_clusters)
    plot(inner_times(left_idx:right_idx),mean(new_pial_clusters(cluster_idx).bc_f_diff,1));
end
