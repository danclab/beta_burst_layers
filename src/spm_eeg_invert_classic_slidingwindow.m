function [D] = spm_eeg_invert_classic_slidingwindow(D,val)
%
...
%     inverse.wois    - time windows of interest ([start stop] in ms)
...
try, wois  = inverse.wois;    catch, wois  = [];       end
...
F=zeros(1,size(wois,1));

if isempty(wois)
    wois = 1000*[min(D.time) max(D.time)];
end
R2=zeros(1,size(wois,1));
VE=zeros(1,size(wois,1));

for w_idx=1:size(wois,1)
    woi=wois(w_idx,:);
    ...
    It     = (woi/1000 - D.timeonset)*D.fsample + 1;
    ...
    for j = 1:Ntrialtypes,                          % pool over conditions
        ...
        for k = 1:Nk
            Y     = A*D(Ic,It,c(k));
            %Y     = A*repmat(mean(D(Ic,It,c(k)),2),[1 length(It) 1]);
    ...
    V      = U(:,1:min([size(U,2) Nr]));                     % temporal modes
    VE(w_idx)     = sum(E(1:min([size(E,2) Nr])));                  % variance explained
    ...
    switch(type)
        ...
        case {'MSP','ARD'}
            ...
            [Cy,h,Ph,F(w_idx)] = spm_sp_reml(AYYA,[],[Qe LQpL],Nn);
    ...
    switch(type)
        ...
        case {'IID','MMN','LOR','COH','EBB','EBBsparse'}
            ...
            [Cy,h,Ph,F(w_idx)] = spm_reml_sc(AYYA,[],[Qe LQpL],Nn,-4,16,Q0);
    ...
    [Cy,h,Ph,F(w_idx)]= spm_reml_sc(AYYA,[],Q,Nn,-4,16,Q0);
    ...
    %======================================================================
    R2(w_idx)   = 100*(SST - SSR)/SST;
    fprintf('Percent variance explained %.2f (%.2f)\n',full(R2(w_idx)),full(R2(w_idx)*VE(w_idx)));
end
...
inverse.woi    = wois;                    % time-window inverted
