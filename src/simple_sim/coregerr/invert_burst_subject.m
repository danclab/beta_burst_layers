function clusters=invert_burst_subject(subj_info, sim_idx, type, coregerr, varargin)
...
min_val=min(mesh_results.cdata(mask));
max_val=max(mesh_results.cdata(mask));
cluster_mask=intersect(mask,find(mesh_results.cdata(:)>=min_val+(max_val-min_val)*percent_thresh));
