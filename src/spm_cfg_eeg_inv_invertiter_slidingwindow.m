function invert = spm_cfg_eeg_inv_invertiter_slidingwindow
...
wois = cfg_entry;
wois.tag = 'wois';
wois.name = 'Time windows of interest';
wois.strtype = 'r';
wois.num = [Inf 2];
wois.val = {[-Inf Inf]};
wois.help = {'Time windows to include in the inversion (ms)'};
...
custom.val  = {invfunc,invtype, wois, foi, hanning,isfixedpatch,patchfwhm,mselect,nsmodes,umodes,ntmodes, priors, restrict,outinv};
...
invert = cfg_exbranch;
invert.tag = 'invertiter_slidingwindow';
invert.name = 'Source inversion, iterative (slidnig window)';
...
if isfield(job.isstandard, 'custom')
    ...
    %inverse.wois  = fix([max(min(job.isstandard.custom.wois(:)), 1000*D.time(1)) min(max(job.isstandard.custom.wois(:)), 1000*D.time(end))]);
    inverse.wois  = job.isstandard.custom.wois;
...
for i = 1:numel(job.D)
    ...
    D{i}.inv{val}.inverse.allF=zeros(size(inverse.wois,1),Npatchiter);
...
crosserr=zeros(size(inverse.wois,1),Nblocks,Ntestchans);
allrms=zeros(size(inverse.wois,1),Nblocks,Ntestchans);
allfract=zeros(size(inverse.wois,1),Nblocks,Ntestchans);
crossF=zeros(size(inverse.wois,1),Nblocks);
...
for b=1:Nblocks,
    ...
    [Dout,allmodels,allF] = spm_eeg_invertiter_slidingwindow(D,Npatchiter,funccall,allIp);
    ...
    if Ntestchans>0, %% if there are any channels to predict..
        ...
        crosserr(size(inverse.wois,1),b)=errpred./length(Ik); %per test channel temporal mode per trial
        allrms(size(inverse.wois,1),b)=rmstot./length(Ik); %per test channel temporal mode per trial
        allfract(size(inverse.wois,1),b)=fracterr./length(Ik); %per test channel temporal mode per trial
    end; %if pctest>0
