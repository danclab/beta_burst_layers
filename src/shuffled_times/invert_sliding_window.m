...
defaults = struct('base_dir', '../data/JB_BUTTON_LOCKED_d3_ers',...
    ...
    'coreg',true,'recompute_lgain',true);  %define default values
...
% Data file to load
if params.coreg
    fname='mrcresp_TafdfC_shuffled.mat';
    if strcmp(params.data_type,'evoked')
        fname='rcresp_TafdfC_shuffled.mat';
    end
else
    fname=sprintf('%s_mrcresp_TafdfC_shuffled.mat',mesh_name);
    if strcmp(params.data_type,'evoked')
        fname=sprintf('%s_rcresp_TafdfC_shuffled.mat',mesh_name);
    end
end
...
coreg_fname=fullfile(data_dir, sprintf('%s_mrcresp_TafdfC_shuffled.mat',mesh_name));
