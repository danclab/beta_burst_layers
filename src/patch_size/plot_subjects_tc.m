...
patch_size=[2.5 5 7.5 10];
labels={};
for i=1:length(patch_size)
    labels{i}=sprintf('%.2f mm', patch_size(i));
end
...
for s_idx=1:length(subjects)
    ...
    for i=1:length(patch_size)
        if patch_size(i)==5
            ...
            fname=sprintf('invert_burst_tc_results_%.2f_patch_size.mat', patch_size(i));
        end
...
colors=cbrewer('seq','Blues',length(patch_size)+1);
