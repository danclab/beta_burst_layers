...
n_temporal_modes=[2 4 6];
labels={};
for i=1:length(n_temporal_modes)
    labels{i}=sprintf('%d temp modes', n_temporal_modes(i));
end
...
for s_idx=1:length(subjects)
    ...
    for i=1:length(n_temporal_modes)
        if n_temporal_modes(i)==4
            fname='invert_burst_tc_results.mat';
        else
            fname=sprintf('invert_burst_tc_results_%d_temp_modes.mat', n_temporal_modes(i));
        end
...
colors=cbrewer('seq','Blues',length(n_temporal_modes)+1);
