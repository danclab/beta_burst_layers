function clusters=invert_burst_subject(subj_info, cluster_threshold, varargin)
...
% Data file to load
data_file=fullfile(data_dir, 'mrcresp_TafdfC.mat');
...
if length(params.plot_dir)==0
    params.plot_dir=fullfile('../../output/figures',base_dir_parts{end},...
        ...
        sprintf('cluster_thresh_%.2f', cluster_threshold));
end
...
cluster_mask=intersect(mask,find(mesh_results.cdata(:)>=max(mesh_results.cdata(mask))*cluster_threshold));
...
if params.plot    
    ...
    fig=get(ax,'Parent');
    saveas(fig, fullfile(params.plot_dir, 'pial_evoked_burst_t-Inf_Inf_clusters.png'), 'png');
    ...
    set(ax,'CameraPosition',subj_info.camera_position('topdown'));
    fig=get(ax,'Parent');
    saveas(fig, fullfile(params.plot_dir, 'pial_evoked_burst_t-Inf_Inf.png'), 'png');
    ...
    set(ax,'CameraPosition',subj_info.camera_position('topdown'));
    fig=get(ax,'Parent');
    saveas(fig, fullfile(params.plot_dir, 'pial_evoked_burst_t-Inf_Inf_mask.png'), 'png');
end       

invert_burst_subject_results=[];
invert_burst_subject_results.params=params;
invert_burst_subject_results.percent_thresh=cluster_threshold;
invert_burst_subject_results.data_file=data_file;
invert_burst_subject_results.clusters=clusters;
save(fullfile(data_dir, sprintf('invert_burst_subject_results_%.2f_cluster_thresh.mat',cluster_threshold)), 'invert_burst_subject_results');
